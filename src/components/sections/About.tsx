...
import { TiltedFigure } from "@/components/TiltedFigure";
        ...
        <div className="max-w-5xl mx-auto pb-10">
            <div className="flex flex-col md:grid md:grid-cols-[1.4fr,1fr] gap-10 items-center md:items-start mb-16 animate-fade-up">
                <div className="space-y-6 order-2 md:order-1 pt-4">
                    <div>
                        <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight mb-4 bg-gradient-to-br from-neutral-950 via-neutral-900 to-neutral-700 dark:from-white dark:via-white dark:to-white/50 bg-clip-text text-transparent dark:drop-shadow-[0_0_35px_rgba(239,68,68,0.4)]">
                            About me
                        </h1>
                    ...
                    <div className="space-y-4 text-lg text-muted-foreground leading-relaxed">
                        <p>I'm <span className="text-foreground font-semibold">{resume.name}</span>, a {resume.role} based in {resume.location}.</p>
                        <p>
                            {resume.bio[0]}{" "}
                            <span className="text-primary font-mono font-bold inline-block whitespace-nowrap min-w-[15ch]">
                                {typeText}
                            </span>
                        </p>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {resume.skills.map((skill) => (
                            <Badge
                                key={skill}
                                variant="secondary"
                                className="px-3 py-1.5 text-sm border border-black/10 dark:border-white/10 bg-black/[0.03] dark:bg-white/5 hover:bg-black/[0.07] dark:hover:bg-white/10 transition-colors shadow-none"
                                style={{ color: `hsl(${languageHsl[skill] ?? "0 0% 50%"})` }}
                            >
                                {skill}
                            </Badge>
                        ))}
                ...
                <article className="glass-card rounded-[1.5rem] p-8 animate-fade-up delay-400">
                    ...
                    <div className="grid gap-4">
                        {resume.attributes.map((attr) => {
                            ...
                            return (
                                <div key={attr.label} className="flex items-start gap-3 p-3 rounded-xl bg-black/[0.03] dark:bg-white/5 border border-black/5 dark:border-white/5 hover:border-black/10 dark:hover:border-white/10 transition-colors group">
                                    <div className={`mt-0.5 p-1.5 rounded-lg bg-black/[0.04] dark:bg-white/5 group-hover:bg-black/[0.08] dark:group-hover:bg-white/10 transition-colors ${attr.color}`}>
